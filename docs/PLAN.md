...
### Estado del juego (Fase 10 — pulido visual y audio, VERIFICADO ✓)
- **Pulido "para que dé ganas de jugar" implementado y verificado en runtime** (petición del usuario en sesión): reemplazo de TODO el arte plano (Polygon2D) por sprites pixel-art procedurales + juiciness + audio rediseñado.
  - **`scripts/pixel_art.gd`** (clase `PixelArt`): genera sprites pixel-art procedurales desde ascii (paletas 16-24 cols, escala 8x). API: `SPRITES` (dict de ascii), `make_texture(key, pal, scale)`, `make_sprite(...)` (Sprite2D con centroide + sombra opcional), `make_sprite_centered(...)`, `_setup_body(...)`, `make_puff(...)`. **Firmas con `rows: Array` SIN tipar** (`Array[String]` rompe porque `SPRITES[...]` devuelve `Array`).
  - **Terreno** (`world.gd`): 3 variantes por bioma con dither determinista (hash de la celda), bordes entre biomas con `EDGE_SOURCE=6`/`EDGE_TILES` (transición suave base↔bioma), `_draw_tile`/`_draw_edge`, decoración procedural (~100 sprites: pasto, flores, arbustos, piedras; rng.seed=4242, densidad 0.14).
  - **Recursos** (`resource_node.gd`): sprites por tipo con 2 variantes (árboles TREE_1/TREE_2, STUMP, ROCK/ROCK_DEAD, MINERAL/MINERAL_DEAD, BUSH/BUSH_DEAD, DEER); agotados muestran la variante "muerta". `play_harvest_fx()` = shake 3 loops + 6 partículas (CPUParticles2D).
  - **Aspectos** (`aspect.gd` + ent/golem/boar/dryad): `_setup_body()` sprites pixel-art + **ojos** (Ent/Dryad: EYES) o **núcleo** (Golem: core brillante) + sombra. Animación: flip horizontal + bob al moverse (16 bobs/s en `_move_toward`).
  - **Jugador** (`player.gd`): Sprite2D pixel-art (2 frames de caminata alternados cada 0.14s), sombra, polvo (CPUParticles2D `_puff`) al caminar/recolectar, `z_index = int(global_position.y)` (también en aspectos por frame; recursos/estaciones/mercader/decor lo fijan al spawnear).
  - **Estaciones + Mercader** (`station.gd`, `merchant.gd`): sprites con sombra (mercantil con toldo a rayas), sin pads de rombos.
  - **Audio rediseñado** (`audio_manager.gd`): sintetizador multicapa (osciladores sine/tri/square + armónicos + ruido filtrado + ADSR + slides de tono) → 11 SFX y **música de 12s en loop Am–F–C–G** (bajo + pads + arpegio + percusión). Mismo autoload `Audio`, misma API (`play_sfx`, `apply_settings`, `debug_state`).
- **Verificación runtime (PID 20472):** capturas `runtime_check1.png` (base) y `runtime_check2.png` (bosque) muestran el arte nuevo; árbol de runtime con 669 nodos (todos los sprites/sombras en su lugar); recolección con FX; muerte por Ent (combate + pérdida OK); **audio verificado en vivo**: `debug_state` → `playing=true` (música con loop), `play_sfx("death")` → Sfx.playing=true (el audio se verificó con un test headless `tests/audio_test.gd` que imprime data/len de cada WAV; ¡cuidado: los sonidos <1s ya terminaron cuando llega la siguiente llamada MCP — comprobar con `playing` en paralelo o sonidos largos!).
- **Lecciones técnicas (Godot 4.6 / MCP):**
  - Al crear un `class_name` nuevo hay que regenerar el cache global de clases (`--headless --editor --quit` una vez) antes de correr headless (si no: "Identifier not declared").
  - Ediciones grandes en archivos con tabs (world.gd): vigilar la indentación — 2 errores de parseo por tabs perdidos (línea 205 y bloque 207-220 del `while`).
  - `AudioStreamWAV.loop_end` está en samples (data.size()/2 para 16-bit mono); valores en bytes se claman solos pero es mejor no confiarse.
  - El estado `playing` de un `AudioStreamPlayer` con sonidos cortos es difícil de leer por la latencia de las llamadas MCP (el sonido ya terminó): leer en paralelo o usar sonidos largos/música.
- Siguiente tarea: **exportar a Web (HTML5) + GitHub Pages** (la meta del POC está implementada y el ciclo completo verificado).
